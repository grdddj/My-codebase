...
let wordCount = []; // array of integers, storing the occurrences of unique words

let alphabetEng = "abcdefghijklmnopqrstuvwxyz"; // string, english alphabet
...
function getNewContent() {
  let helper = "https://cors-anywhere.herokuapp.com/"; // string used to enable fetching content from the web with the use of third party server
  let url = $("#url").val(); // getting the url from user
...
function countWords(allWords) {
  ...
  for (let i = 0; i < allWords.length; i++) { //going through all the words
      let isThere = false; // boolean to know whether word is already stored in uniqueWords
