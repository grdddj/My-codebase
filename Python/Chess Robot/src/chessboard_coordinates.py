...
class ChessboardCoordinates(ChessboardCoordinatesInterface):
    def __init__(
        ...
        self._square_size = (right_bottom[0] - left_top[0]) // 8
        assert self._square_size > 0, "Invalid square size. Check coordinates"
        self._square_centers_dict = self._create_dict_of_square_centers(
            left_top, right_bottom, our_piece_colour
        )

    def get_square_center(self, square: Square) -> "Pixel":
        if square not in self._square_centers_dict:
            raise ValueError(f"Square {square} does not exist!")

        return self._square_centers_dict[square]

    def get_all_square_items(self) -> Iterable[tuple[Square, "Pixel"]]:
        return self._square_centers_dict.items()
    ...
    def _create_dict_of_square_centers(
        ...
        assert our_piece_colour in ("white", "black"), "Invalid colour"
