...
logging.info(
    "{} - We are starting - clicks: {}".format(begin_timestamp, amount_of_next_clicks)
)
...
try:
    ...
    fetch_comments_for_all_articles_from_file_into_a_folder(
        full_file_path, full_folder_name
    )
    ...
    logging.info(
        "{} - We are finished. Took {} s to get {} pages".format(
            begin_timestamp,
            int(time.time()) - begin_timestamp,
            amount_of_next_clicks + 1,
        )
    )
