    ...
    with open("conversations.csv", "r") as conversations_table:
        ...
        for entry in csv_reader:
            if entry["user_1_name"] == USER_NAME:
                contacts.append(
                    {"name": entry["user_2_name"], "conv_id": entry["conv_id"]}
                )
            elif entry["user_2_name"] == USER_NAME:
                contacts.append(
                    {"name": entry["user_1_name"], "conv_id": entry["conv_id"]}
                )
            ...
            if entry["name"] in all_contact_names:
                for index, contact in enumerate(contacts):
                    if contact["name"] == entry["name"]:
                        contacts[index]["last_time_online"] = int(
                            entry["last_time_online"]
                        )
...
def populate_conversation(conversation_id, user_name, user_mood):
    ...
    file_path = "Conversations/{}-{}.csv".format(
        USER_NAME, CURRENTLY_OPENED_CONTACT_NAME
    )
    ...
    with open(file_path, "r") as messages_file:
        ...
        for message in csv_reader:
            text_messages += "{}: {}\n".format(
                message["user_name"], message["message_text"]
            )
    ...
    user_number = (
        1
        if conversations_table.loc[
            conversations_table["conv_id"] == conversation_id, "user_1_name"
        ].values[0]
        == USER_NAME
        else 2
    )
    ...
    last_message_timestamp = int(
        conversations_table.loc[
            conversations_table["conv_id"] == conversation_id, "last_message_timestamp"
        ].values[0]
    )
    last_displayed_timestamp = int(
        conversations_table.loc[
            conversations_table["conv_id"] == conversation_id,
            "last_user_{}_read_timestamp".format(other_user_number),
        ].values[0]
    )
    last_message_user_name = conversations_table.loc[
        conversations_table["conv_id"] == conversation_id, "last_message_user_name"
    ].values[0]
    last_typing_timestamp = int(
        conversations_table.loc[
            conversations_table["conv_id"] == conversation_id,
            "last_user_{}_typing_timestamp".format(other_user_number),
        ].values[0]
    )
    ...
    conversations_table.loc[
        conversations_table["conv_id"] == conversation_id,
        "last_user_{}_read_timestamp".format(user_number),
    ] = current_timestamp
    ...
    if messaging_text.get("1.0", "end-1c") != "":
        conversations_table.loc[
            conversations_table["conv_id"] == conversation_id,
            "last_user_{}_typing_timestamp".format(user_number),
        ] = current_timestamp
...
def send_message(message, conversation_id):
    ...
    current_timestamp = int(time.time())
    human_readable_time = datetime.datetime.fromtimestamp(current_timestamp).strftime(
        "%Y-%m-%d %H:%M:%S"
    )
    ...
    with open("contact_requests.csv", "w") as contact_requests_table_write:
        ...
        for row in csv_reader:
            if len(row) == 0:
                continue
            if not (
                (row[0] == user_name and row[1] == USER_NAME)
                or (row[0] == USER_NAME and row[1] == user_name)
            ):
...
def send_contact_request(user_name):
    ...
    with open("conversations.csv", "r") as conversations_table:
        ...
        for entry in csv_reader:
            if (
                entry["user_1_name"] == user_name and entry["user_2_name"] == USER_NAME
            ) or (
                entry["user_1_name"] == USER_NAME and entry["user_2_name"] == user_name
            ):
...
def update_number_of_requests():
    ...
    contact_requests_button["text"] = "Manage requests ({})".format(
        len(list_of_users_requesting_contact)
    )
...
def log_last_time_online(user_name):
    ...
    users_table = pd.read_csv("users.csv")
    users_table.loc[
        users_table["name"] == user_name, "last_time_online"
    ] = current_timestamp
    users_table.to_csv("users.csv", index=False)
