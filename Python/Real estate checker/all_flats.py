...
def get_content():
    ...
    logger.log_info("Number of results: {}".format(number_of_results))
    ...
    logger.log_info("Number of fetched results: {}".format(len(estates)))
    return estates


def get_number_of_results():
    # Testing request to get the amount of results
    test_url = ("{}?category_main_cb=1&category_type_cb=1"
...
def get_new_estates(page_num, results_per_page):
    url = ("{}?category_main_cb=1&category_type_cb=1"
           "&locality_region_id=14&page={}&per_page={}&sort=1"
           ).format(api_endpoint, page_num, results_per_page)
...
def analyze_content(estates):
    ...
    for flat_id, info in flat_dict.items():
        if flat_id not in last_results:
            logger.log_info("Adding new flat: id {}".format(flat_id))
            ...
            if info["price"] != last_results[flat_id]["price"]:
                info_msg = "Price changed (id {}):from {} to {}".format(
...
def get_flat_dict(estates):
    ...
    for flat in estates:
        ...
        link = "https://www.sreality.cz/detail/prodej/byt/{}/{}/{}".format(
            type, locality_criptic, flat_id)
...
def archive_last_results():
    ...
    backup_name = "results_backup_{}.json".format(now_ts)
    backup_path = os.path.join(backup_dir, backup_name)

    logger.log_info("Backing up results - {}".format(backup_path))
