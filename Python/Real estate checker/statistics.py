...
class Statistics:
    ...
    def overall_number_of_flats_we_have(self) -> int:
    ...
    def _changed_prices(self) -> Flats:
        key = "price"
        return self._changed_value_of(key)

    def display_details_of_changed_prices(self) -> None:
        ...
        for flat in changed_prices_flats_sorted:
            ...
            relevant_price_history = [
                entry for entry in flat["price_history"] if entry["price"] != 1
            ]
            if len(relevant_price_history) < 2:
                continue

            history_values = [
                f"{entry['date']} - {entry['price']:,}"
                for entry in relevant_price_history
            ]
            ...
            percentage_change = round(
                100 * (last_price - previous_price) / previous_price, 2
            )

            last_change = f"{absolute_change} ({percentage_change} %)"

            data.append(
                [info["info"], price_history_string, last_change, info["link_to_show"]]
            )
    ...
    def _get_sorted_by_last_change(self, field_of_change: str, flats: Flats) -> Flats:
        return sorted(
            flats,
            key=lambda flat: helpers.get_timestamp_from_formatted_date(
                flat[field_of_change][-1]["date"]
            ),
        )

    def some_offer_state(self) -> None:
        results = self._get_results()
        key = "state_of_offer"
    ...
    def _changed_value_of(self, key: str) -> Flats:
        key_history = f"{key}_history"
        results = self._get_results()
    ...
    def _get_average_price(self, results: Flats) -> int:
        number_of_results = len(results)
    ...
    def _get_median_price(self, results: Flats) -> int:
    ...
    @staticmethod
    def aggregate_all_possible_values(results, key: str) -> None:
    ...
    def not_active_anymore(self) -> Flats:
        today_date = datetime.now().strftime("%Y-%m-%d")
        results = self._get_results()
        ...
        matching_results = self._get_results_with_different_key_value(
            results, key, value
        )
    ...
    def _get_results_with_specific_key(self, results, key) -> Flats:
        matching_results = [result for result in results if key in result]
        return matching_results

    def _get_results_with_exact_key_value(self, results, key, value) -> Flats:
        matching_results = [result for result in results if result.get(key) == value]
        return matching_results

    def _get_results_with_different_key_value(self, results, key, value) -> Flats:
        matching_results = [result for result in results if result.get(key) != value]
        return matching_results

    def show_flat_containing_some_text(self) -> Flats:
        text = input("Text: ")
        return self.flat_containing_some_text(text)

    def flat_containing_some_text(self, text: str) -> Flats:
        text_pattern = re.compile(rf"{text}", re.IGNORECASE)
        results = self._get_results()
        matching_results = [
            result for result in results if text_pattern.search(str(result))
        ]
    ...
    def show_all_information_in_a_table(self, results: Flats) -> None:
        data = []
        for flat in results:
            info = self._get_basic_info(flat)
            description = flat["description"]

            description = self.fit_text_into_column_with_character_limit(
                description, 60
            )
            link_to_show = self.fit_text_into_column_with_character_limit(
                info["link_to_show"], 610
            )
    ...
    @staticmethod
    def include_horizontal_delimiter(text: str) -> str:
    ...
    @staticmethod
    def fit_text_into_column_with_character_limit(text: str, line_limit=60) -> str:
        new_lines = []
    ...
    def show_specific_flat(self) -> Flat:
        flat_id = input("Flat ID: ")
        return self.get_specific_flat(flat_id)

    def get_specific_flat(self, flat_id: str) -> Flat:
        results = self._get_results()
    ...
    def new_flats_each_day(self) -> None:
        results = self._get_results()
        self._aggregate_info_by_time(results, "first_seen_date")

    def sold_flats_each_day(self) -> None:
        results = self._get_results()
        self._aggregate_info_by_time(results, "last_update_date")

    def sold_last_week(self) -> None:
        results = self._get_results()

        utc_dt_object = datetime.now()
        day_start = datetime(
            year=utc_dt_object.year,
            month=utc_dt_object.month,
            day=utc_dt_object.day,
            hour=0,
            second=0,
        )
        ...
        results = [
            result for result in results if result["last_update_ts"] > week_ago_ts
        ]
        results = [
            result for result in results if result["last_update_ts"] < day_start_ts
        ]
    ...
    def new_last_week(self) -> None:
        ...
        results = [
            result for result in results if result["first_seen_ts"] > week_ago_ts
        ]
    ...
    def _aggregate_info_by_time(self, results, key) -> None:
    ...
    def _get_basic_info(self, flat: Flat) -> dict:
        type = flat["type"]
        ...
        return {"info": info, "price": price, "link_to_show": link_to_show}

    @staticmethod
    def _print_horizontal_divide() -> None:
        print(80 * "*")
...
if __name__ == "__main__":
    ...
    query_to_function = [
        ...
        (
            "Flats changed prices recently",
            "changed_price",
            "display_details_of_changed_prices",
        ),
    ...
    def call_function_according_to_number_input(number: str) -> None:
        try:
    ...
    def call_function_according_to_string_input(string: str) -> None:
    ...
    def show_all_possibilities() -> None:
