...
c = conn.cursor()
c.execute('''CREATE TABLE IF NOT EXISTS hates
             (id integer primary key, hater_name text, other_person_name text, reason text)''')
c.execute('''CREATE TABLE IF NOT EXISTS loves
...
class Hate(Resource):
    def post(self):
        ...
        try:
            c.execute("INSERT INTO hates (hater_name, other_person_name, reason) VALUES (?, ?, ?)",
...
class Love(Resource):
    def post(self):
        ...
        try:
            c.execute("INSERT INTO loves (lover_name, other_person_name, reason) VALUES (?, ?, ?)",
...
class HateLeaderboard(Resource):
    def get(self):
        ...
        c.execute("SELECT other_person_name, COUNT(other_person_name) as count FROM hates GROUP BY other_person_name ORDER BY 2 DESC")
        result = c.fetchall()
...
class LoveLeaderboard(Resource):
    def get(self):
        ...
        c.execute("SELECT other_person_name, COUNT(other_person_name) as count FROM loves GROUP BY other_person_name ORDER BY 2 DESC")
        result = c.fetchall()
